package ko.inflearnspringsecurity.common;

import ko.inflearnspringsecurity.account.Account;
import ko.inflearnspringsecurity.account.AccountService;
import ko.inflearnspringsecurity.book.Book;
import ko.inflearnspringsecurity.book.BookRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.ApplicationArguments;
import org.springframework.boot.ApplicationRunner;
import org.springframework.stereotype.Component;

@Component
public class DefaultDataGenerator implements ApplicationRunner {

    @Autowired
    AccountService accountService;

    @Autowired
    BookRepository bookRepository;

    @Override
    public void run(ApplicationArguments args) throws Exception {
        //aaa - spring
        //bbb - jpa
        Account aaa = createUser("aaa");
        Account bbb =createUser("bbb");

        createBook("spring",aaa);
        createBook("jpa",bbb);
    }

    private void createBook(String title ,Account username) {
        Book book = new Book();
        book.setTitle(title);
        book.setAuthor(username);
        bookRepository.save(book);
    }

    private Account createUser(String username) {
        Account account = new Account();
        account.setUsername(username);
        account.setPassword("123");
        account.setRole("USER");
        return accountService.createNew(account);
    }

}
